...
2. **選深度**（`.claude/harness/06-dev-workflow.md` 站 4）：S 級輕量三項（read-back／驗證命令 exit 0／無夾帶改動）；M 級＝以 03 模板 4 完整執行（**驗收即審查**，站 5 不再另派）；L 級＝01 §5 對應手段＋`.claude/harness/02-judgment-matrix.md` 第 2 表逐條（獨立審查留給站 5）。
