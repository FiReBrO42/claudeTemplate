...
### Subagent 在受限情境下採用的權宜解，容易被它自己落檔成「無條件正解」——落檔教訓須經 fresh-context 審查校準
- 〔症狀〕WORKER 因既有凍結測試以無 router mount 直掛、RouterLink 會崩，改用原生 `<a href>` 繞過，並把此權宜解落檔成教訓的「正確做法」，未揭露其代價（history 模式整頁重載丟語系）、也未評估同樣可行且更優的 `useRouter()+push`；若無獨立審查，下一個任務會把權宜解當正解引用
- 〔觸發條件/重現步驟〕subagent 在受限情境（凍結測試、相容性約束、外部限制）下自主選定解法並自行落檔教訓；COMMANDER 未對其落檔內容做獨立校準即採信
- 〔根因〕subagent 是全新 context 且目標是「過驗收」，傾向採最短可行路徑，落檔時會把自身的權宜選擇合理化為「正解」，缺乏對更優解的探索與對代價的揭露；「能過測試」不等於「最佳解」，更不等於「可傳承的通用正解」
- 〔正確做法〕COMMANDER 對 subagent 落檔的教訓（尤其在受限情境下產生者）一律過 fresh-context 審查校準：要求教訓標明①權宜解的具體代價②是否存在更優解及其可行性③以「優先序/三層方案」而非單一「正解」呈現；審查發現失真即退回更正並雙寫同步。與「並行收報須核對」同屬「不可盡信 subagent 產出」家族——產出要核對、落檔的知識同樣要校準
- 〔判準〕通用
- 〔日期〕2026-07-05
- 〔來源專案〕invest（AdviceView 一鍵帶入入口：WORKER 一度把 `<a href>` 權宜解落檔為正解，經站 5 獨立審查揭露整頁重載代價與 useRouter()+push 更優解後更正記載）
